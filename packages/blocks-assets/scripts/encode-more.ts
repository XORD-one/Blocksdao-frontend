import { PNGCollectionEncoder } from "@blockdao/sdk";
...
const encode = async (sourceFolder: string, destinationFilepath: string) => {
  ...
  const partfolders = ["1-bodies", "2-accessories", "3-caps"];
  ...
  if (JSON.stringify(encoder.data.palette) !== JSON.stringify(palette)) {
    ...
    await fs.writeFile(
      "original_palette.json",
      JSON.stringify(palette, null, 2)
    );
    await fs.writeFile(
      "new_palette.json",
      JSON.stringify(encoder.data.palette, null, 2)
    );
