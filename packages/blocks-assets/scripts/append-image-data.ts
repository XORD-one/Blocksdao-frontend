import { ImageData } from "@blockdao/sdk/dist/image/types";
...
const merge = async (
  ...
  const baseData: ImageData = JSON.parse(
    (await fs.readFile(baseFile)).toString()
  );
  const newData: ImageData = JSON.parse(
    (await fs.readFile(newFile)).toString()
  );
