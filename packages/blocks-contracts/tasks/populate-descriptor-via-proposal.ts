...
task(
  ...
  .addParam('blocksDescriptor', 'The `BlocksDescriptor` contract address')
  .addParam('daoAddress', 'The `BlocksDAOProxy` contract address')
  ...
  .setAction(
    async ({ blocksDescriptor, daoAddress, imageDataPath, proposalTextPath }, { ethers }) => {
      ...
      const dao = (await ethers.getContractFactory('BlocksDAOLogicV1')).attach(daoAddress);
