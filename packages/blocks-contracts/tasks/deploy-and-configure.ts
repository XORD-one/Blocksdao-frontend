...
task('deploy-and-configure', 'Deploy and configure all contracts')
  ...
  .setAction(async (args, { run }) => {
    // Deploy the Blocks DAO contracts and return deployment information
    ...
    const executorAddress = contracts.BlocksDAOExecutor.address;
    ...
    // to the Blocks DAO executor.
    if (args.startAuction) {
      const auctionHouse = contracts.BlocksAuctionHouse.instance.attach(
        contracts.BlocksAuctionHouseProxy.address,
      );
