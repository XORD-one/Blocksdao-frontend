...
task('deploy-test-token', 'Deploy BlocksTokenHarness given a descriptor')
  ...
  .setAction(async ({ descriptorAddress, seederAddress }, { ethers, run, network }) => {
    ...
    const token = await (
      await ethers.getContractFactory('BlocksTokenHarness', deployer)
    ).deploy(
      deployer.address,
      deployer.address,
      descriptorAddress,
    ...
    console.log(`BlocksTokenHarness deployed to: ${token.address}`);
    ...
    if (network.name !== 'localhost') {
      ...
      contracts.BlocksTokenHarness = {
        name: 'BlocksTokenHarness',
