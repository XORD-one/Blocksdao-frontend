...
task('deploy-descriptor-v2', 'Deploy BlocksDescriptorV2 & populate it with art')
  .addParam(
    'daoExecutor',
    'The address of the BlocksDAOExecutor that should be the owner of the descriptor.',
  )
  .setAction(async ({ daoExecutor }, { ethers, run, network }) => {
    ...
    const blocksDescriptorFactory = await ethers.getContractFactory('BlocksDescriptorV2', {
      libraries: {
        NFTDescriptorV2: library.address,
      },
    });
    const blocksDescriptor = await blocksDescriptorFactory.deploy(
      expectedBlocksArtAddress,
      renderer.address,
    );
    contracts.BlocksDescriptorV2 = {
      name: 'BlocksDescriptorV2',
      address: blocksDescriptor.address,
      constructorArguments: [expectedBlocksArtAddress, renderer.address],
      instance: blocksDescriptor,
      libraries: {
        NFTDescriptorV2: library.address,
      },
    };
    ...
    const art = await (
      await ethers.getContractFactory('BlocksArt', deployer)
    ).deploy(blocksDescriptor.address, inflator.address);
    contracts.BlocksArt = {
      name: 'BlocksArt',
      address: art.address,
      constructorArguments: [blocksDescriptor.address, inflator.address],
      instance: art,
      libraries: {},
    };
    ...
    await blocksDescriptor.transferOwnership(daoExecutor);
