...
task('deploy-short-times-daov1', 'Deploy all Blocks contracts with short gov times for testing')
  ...
  .setAction(async (args, { ethers }) => {
    ...
    const contracts: Record<ContractName, ContractDeployment> = {
      ...
      SVGRenderer: {},
      BlocksDescriptorV2: {
        args: [expectedBlocksArtAddress, () => deployment.SVGRenderer.address],
        libraries: () => ({
      ...
      Inflator: {},
      BlocksArt: {
        args: [() => deployment.BlocksDescriptorV2.address, () => deployment.Inflator.address],
      },
      BlocksSeeder: {},
      BlocksToken: {
        args: [
          ...
          () => deployment.BlocksDescriptorV2.address,
          () => deployment.BlocksSeeder.address,
      ...
      BlocksAuctionHouseProxy: {
        args: [
          () => deployment.BlocksAuctionHouse.address,
          () => deployment.BlocksAuctionHouseProxyAdmin.address,
          () =>
            new Interface(BlocksAuctionHouseABI).encodeFunctionData('initialize', [
              deployment.BlocksToken.address,
              args.weth,
        ...
        validateDeployment: () => {
          ...
          const actual = deployment.BlocksAuctionHouseProxy.address.toLowerCase();
          if (expected !== actual) {
            throw new Error(
      ...
      BlocksDAOProxy: {
        args: [
          () => deployment.BlocksDAOExecutor.address,
          () => deployment.BlocksToken.address,
          args.noundersdao,
          () => deployment.BlocksDAOExecutor.address,
          () => deployment.BlocksDAOLogicV1.address,
          args.votingPeriod,
          args.votingDelay,
        ...
        waitForConfirmation: true,
        validateDeployment: () => {
          const expected = expectedBlocksDAOProxyAddress.toLowerCase();
          const actual = deployment.BlocksDAOProxy.address.toLowerCase();
          if (expected !== actual) {
            throw new Error(
              `Unexpected Blocks DAO proxy address. Expected: ${expected}. Actual: ${actual}.`,
            );
          }
        },
      },
    ...
    for (const [name, contract] of Object.entries(contracts)) {
      ...
      switch (name) {
        case 'BlocksDAOExecutor':
          nameForFactory = 'BlocksDAOExecutorTest';
          break;
        case 'BlocksDAOLogicV1':
          nameForFactory = 'BlocksDAOLogicV1Harness';
          break;
        default:
          nameForFactory = name;
          break;
