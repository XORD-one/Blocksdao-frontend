...
///
/// @title The BlocksDao NFT descriptor

pragma solidity ^0.8.6;

import { Ownable } from "@openzeppelin/contracts/access/Ownable.sol";
import { Strings } from "@openzeppelin/contracts/utils/Strings.sol";
import { IBlocksDescriptor } from "./interfaces/IBlocksDescriptor.sol";
...
contract BlocksDescriptor is IBlocksDescriptor, Ownable {
    ...
    /// @notice The contract responsible for holding compressed block art
    IBlocksArt public art;

    /// @notice The contract responsible for constructing SVGs
    ISVGRenderer public renderer;

    /// @notice Whether or not new block parts can be added
    ...
    constructor(IBlocksArt _art, ISVGRenderer _renderer) {
        art = _art;
        renderer = _renderer;
    }

    /**
     * @notice Set the Block's art contract.
     * @dev Only callable by the owner when not locked.
     */
    function setArt(IBlocksArt _art) external onlyOwner whenPartsNotLocked {
        art = _art;
    ...
    /**
     * @notice Get the number of available block `bodies`.
     */
    ...
    /**
     * @notice Get the number of available block `accessories`.
     */
    ...
    /**
     * @notice Get the number of available block `capStyless`.
     */
    function capStyleCount() external view override returns (uint256) {
        return art.getCapStylesTrait().storedImagesCount;
    }
    ...
    /**
     * @notice Add a batch of capStyles images.
     * @param encodedCompressed bytes created by taking a string array of RLE-encoded images, a
     bi encoding it as a bytes array,
     * and finally compressing it using deflate.
     * @param decompressedLength the size in bytes the images bytes were prior to compression; 
     required input for Inflate.
     * @param imageCount the number of images in this batch; used when searching for images among batches.
    ...
    function addCapStyles(
        ...
        art.addCapStyles(encodedCompressed, decompressedLength, imageCount);
    }
    ...
    /**
     * @notice Add a batch of capStyles images from an existing storage contract.
     * @param pointer the address of a contract where the image batch was stored using SSTORE2. The data
     * format is expected to be like {encodedCompressed}: bytes created by taking a string array of
     * RLE-encoded images, abi encoding it as a bytes array, and finally compressing it using deflate.
     * @param decompressedLength the size in bytes the images bytes were prior to compression;
    ...
    function addCapStylesFromPointer(
        ...
        art.addCapStylesFromPointer(pointer, decompressedLength, imageCount);
    }
    ...
    /**
     * @notice Get a capStyles image by ID.
     * @param index the index of the capStyles.
    ...
    /**
     * @notice Given a token ID and seed, construct a token URI for an official blocks DAO block.
    ...
    /**
     * @notice Given a token ID and seed, construct a base64 encoded data URI for an official blocksDAO block.
     */
    function dataURI(uint256 tokenId, IBlocksDaoSeeder.Seed memory seed) public view override returns (string memory) {
        string memory blockId = tokenId.toString();
        string memory name = string(abi.encodePacked("block ", blockId));
        string memory description = string(abi.encodePacked("block ", blockId, " is a member of the blocks DAO"));
     ...
     */
    function generateSVGImage(IBlocksDaoSeeder.Seed memory seed) external view override returns (string memory) {
    ...
    /**
     * @notice Get all block parts for the passed `seed`.
     */
    function getPartsForSeed(IBlocksDaoSeeder.Seed memory seed) public view returns (ISVGRenderer.Part[] memory) {
        bytes memory body = art.bodies(seed.body);
        bytes memory accessory = art.accessories(seed.accessory);
        bytes memory capStyle = art.capStyles(seed.capStyle);
        ...
        parts[2] = ISVGRenderer.Part({ image: capStyle, palette: _getPalette(capStyle) });
        return parts;
    }
