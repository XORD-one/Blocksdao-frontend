...
import { ImageData, getBlockData, getRandomBlockSeed } from "@blockdao/assets";
import { buildSVG, EncodedImage, PNGCollectionEncoder } from "@blockdao/sdk";
...
const Playground: React.FC = () => {
  ...
  const generateNounSvg = React.useCallback(
    (amount: number = 1) => {
      for (let i = 0; i < amount; i++) {
        const seed = { ...getRandomBlockSeed(), ...modSeed };
  ...
  const validateAndSetCustomTrait = (file: File | undefined) => {
    ...
    reader.onload = (e) => {
      try {
        const buffer = Buffer.from(e?.target?.result! as any);
      ...
      <Container fluid="lg">
        <Row>
          <Col lg={10} className={classes.headerRow}>
            ...
            <p>
              <Trans>
                The playground was built using the {nounsProtocolLink}. Noun's
                traits are determined by the Noun Seed. The seed was generated
                using {nounsAssetsLink} and rendered using the {nounsSDKLink}.
              </Trans>
            </p>
          </Col>
        </Row>
