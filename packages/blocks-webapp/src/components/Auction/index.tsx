...
import { IBlockSeed } from "../../wrappers/nounToken";
...
const Auction: React.FC<AuctionProps> = (props) => {
  ...
  const loadedNounHandler = (seed: IBlockSeed) => {
    dispatch(setStateBackgroundColor(seed.background === 0 ? grey : beige));
  };

  const prevAuctionHandler = () => {
    dispatch(setPrevOnDisplayAuctionBlockId());
    currentAuction &&
      history.push(`/noun/${currentAuction.blockId.toNumber() - 1}`);
  };
  const nextAuctionHandler = () => {
    dispatch(setNextOnDisplayAuctionBlockId());
    currentAuction &&
      history.push(`/noun/${currentAuction.blockId.toNumber() + 1}`);
  };
  ...
  const currentAuctionActivityContent = currentAuction && lastNounId && (
    <AuctionActivity
      ...
      isFirstAuction={currentAuction.blockId.eq(0)}
      isLastAuction={currentAuction.blockId.eq(lastNounId)}
      ...
      <Container fluid="xl">
        <Row>
          ...
          <Col lg={{ span: 6 }} className={classes.auctionActivityCol}>
            {currentAuction &&
              (isNounderNoun(currentAuction.blockId) 
                ? nounderNounContent 
