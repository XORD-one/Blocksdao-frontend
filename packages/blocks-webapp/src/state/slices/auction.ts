...
export const reduxSafeNewAuction = (auction: AuctionCreateEvent): IAuction => ({
  ...
  blockId: BigNumber.from(auction.blockId).toJSON(),
  settled: false,
});
...
export const reduxSafeBid = (bid: BidEvent): BidEvent => ({
  blockId: BigNumber.from(bid.blockId).toJSON(),
  sender: bid.sender,
...
) => BigNumber.from(a.blockId).eq(BigNumber.from(b.blockId));
...
export const auctionSlice = createSlice({
  ...
  reducers: {
    ...
    setAuctionSettled: (state, action: PayloadAction<AuctionSettledEvent>) => {
      ...
      state.activeAuction.amount = BigNumber.from(
    ...
    setAuctionExtended: (
      ...
      if (
        ...
        return;
      state.activeAuction.endTime = BigNumber.from(
