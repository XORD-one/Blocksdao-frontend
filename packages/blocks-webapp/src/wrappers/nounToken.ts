...
import { BlocksTokenFactory, BlocksTokenABI } from "@blockdao/contracts";
...
import { BlocksToken_Address } from "../contracts/BlocksToken";
...
const abi = new utils.Interface(BlocksTokenABI);
// const seedCacheKey = cacheKey(cache.seed, CHAIN_ID, BlocksToken_Address);

// const isSeedValid = (seed: Record<string, any> | undefined) => {
//   // check out line no 38
...
export const useDelegateVotes = () => {
  const blocksToken: any = new BlocksTokenFactory().attach(BlocksToken_Address);

  const { send, state } = useContractFunction(blocksToken, "delegate");
